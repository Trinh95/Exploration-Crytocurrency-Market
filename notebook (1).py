...
# Filtering out rows without a market capitalizatio
cap = market_cap_raw.query("market_cap_usd != 'NaN'")
...
cap10 = cap.sort_values('market_cap_usd',ascending=False)[:10].set_index('id')

# Calculating market_cap_perc

#cap10= cap10.assign(market_cap_perc = lambda x: (cap10['market_cap_usd']/(cap10['market_cap_usd'].sum()))*100)
...
ax=cap10.reset_index().plot(x='id',y='market_cap_perc',kind='bar')
...
ax = cap10.reset_index().plot(x='id',y='market_cap_usd',kind='bar',colors=COLORS)
...
def top10_subplot(volatility_series, title):
    ...
    ax1=volatility_series[-10:].plot.bar(ax=axes[1],colors='darkblue')
...
plt.show()
#print(volatility['percent_change_24h'].head())
...
def capcount(query_string):
    return cap[query_string].count().id

# Labels for the plot

# Using capcount count the biggish cryptos
biggish = cap[cap['market_cap_usd'] >= 300000000].shape[0]
print(biggish)


# Same as above for micro ...
#micro = capcount('market_cap_usd >= 50000000 & market_cap_usd < 300000000')
micro = cap[(cap['market_cap_usd'] >= 50000000) & (cap['market_cap_usd']< 300000000)].shape[0]
# ... and for nano
nano =  cap[cap['market_cap_usd'] < 50000000].shape[0]
